...
def main(username, tree):
    ...
    def submit(firstname_Entry, lastname_Entry, address_entry, email_entry, phone_num_entry, username_Entry,
        ...
        if email_entry.get():
            non_empty_fields.append(("email", email_entry.get()))
        if phone_num_entry.get():
            non_empty_fields.append(("phone", phone_num_entry.get()))
    ...
    bck_btn = Button(frame, text='<<', width=7, borderwidth=5, height=2, bg='#babcbb', fg='black', cursor='hand2', border=2, command= lambda :back_to_profile(username))
